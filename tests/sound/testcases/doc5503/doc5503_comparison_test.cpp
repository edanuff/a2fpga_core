...
// Global timestamp for proper timing emulation
...
int main(int argc, char** argv) {
    ...
    // Initialize wavetable with a sawtooth wave (better for testing, no 0x00 values)
    printf("Initializing wavetable with sawtooth wave pattern...\n");
    for (int i = 0; i < 256; i++) {
        // Use sawtooth pattern that avoids 0x00 (which would halt oscillators)
        wavetable_mem[i] = 0x80 + ((i % 127) + 1); // Range from 0x81-0xFF
    ...
    // Fill the rest with non-zero values (silence is 0x80)
    ...
    // Print ES5503 implementation information
    printf("ES5503 C++ implementation configured:\n");
    printf("- Clock frequency: 7159090 Hz\n");
    printf("- Channels: 2 (stereo)\n");
    printf("- Wavetable size: %d bytes\n", wavetable_size);
    ...
    // Set control register for both implementations
    // For Verilog: 0x00 = Channel 0 (right output), not halted
    // For C++: this gets translated differently - we need to observe actual channel mapping
    // Let's set it to channel 0 and verify where it actually appears
    ...
    // Verify control register setting
    uint8_t control_reg;
    for (int i = 0; i < 2; i++) {
        clk = !clk;
        doc_harness->clk_i = clk;
    ...
    control_reg = doc_harness->data_o;
    printf("Control register value: 0x%02X (Halted: %s, Channel: %d)\n", 
    ...
    for (int i = 0; i < MAX_SIM_TIME; i++) {
        ...
        if (clk) {
            ...
            if (doc_harness->wave_rd_o) {
                ...
                // Always print some wave reads to help diagnose issues
                if (verbosity >= DEBUG || (sim_time < 5000 && sim_time % 500 == 0)) {
                    printf("Time %d: Wave read at address 0x%04x, providing value 0x%02x\n", 
                           sim_time, addr, value);
                }
            } else {
                doc_harness->wave_data_ready_i = 0;
            ...
            if ((sim_time % SAMPLE_COLLECT_INTERVAL) == 0) {
                ...
                // IMPORTANT: Swap left and right channels to match Verilog implementation's channel assignment
                // In the C++ model, oscillator 0 on channel 0 outputs to left channel
                // In the Verilog model, oscillator 0 on channel 0 outputs to right channel
                cpp_samples_right.push_back(audio_buffer[audio_buffer_pos * 2]);      // Left -> Right
                cpp_samples_left.push_back(audio_buffer[audio_buffer_pos * 2 + 1]);   // Right -> Left
...
void compare_audio_outputs(const std::vector<int16_t>& verilog_samples, const std::vector<int16_t>& cpp_samples, const std::string& test_name) {
    ...
    // Check if samples contain any non-zero values
    bool verilog_has_audio = false;
    bool cpp_has_audio = false;
    
    for (auto sample : verilog_samples) {
        if (sample != 0) {
            verilog_has_audio = true;
            break;
        }
    }
    
    for (auto sample : cpp_samples) {
        if (sample != 0) {
            cpp_has_audio = true;
            break;
        }
    ...
    printf("Verilog implementation produces audio: %s\n", verilog_has_audio ? "YES" : "NO");
    printf("C++ implementation produces audio: %s\n", cpp_has_audio ? "YES" : "NO");
    
    // If either implementation isn't producing audio, exit early
    if (!verilog_has_audio || !cpp_has_audio) {
        printf("WARNING: At least one implementation is not producing audio!\n");
        printf("Cannot perform meaningful comparison without audio from both implementations.\n");
        
        // Print the first few non-zero samples from each implementation
        printf("\nSample preview (first 10 non-zero samples):\n");
        printf("Verilog: ");
        int count = 0;
        for (size_t i = 0; i < verilog_samples.size() && count < 10; i++) {
            if (verilog_samples[i] != 0) {
                printf("%d ", verilog_samples[i]);
                count++;
            }
        }
        if (count == 0) printf("No non-zero samples");
        printf("\n");
        
        printf("C++: ");
        count = 0;
        for (size_t i = 0; i < cpp_samples.size() && count < 10; i++) {
            if (cpp_samples[i] != 0) {
                printf("%d ", cpp_samples[i]);
                count++;
            }
        }
        if (count == 0) printf("No non-zero samples");
    ...
    for (int i = 0; i < min_size; i++) {
        ...
        if (verilog_samples[i] != 0) non_zero_v++;
        if (cpp_samples[i] != 0) non_zero_c++;
    }
    ...
    printf("Non-zero samples: Verilog=%d (%.1f%%), C++=%d (%.1f%%)\n", 
           non_zero_v, 100.0 * non_zero_v / verilog_samples.size(),
           non_zero_c, 100.0 * non_zero_c / cpp_samples.size());
    ...
    // Threshold for acceptable difference (adjustable)
    ...
    // Print the first few samples from each implementation for visual comparison
    printf("\nSample preview (first 10 samples with either non-zero):\n");
    printf("Index   Verilog   C++\n");
    int preview_count = 0;
    for (size_t i = 0; i < min_size && preview_count < 10; i++) {
        if (verilog_samples[i] != 0 || cpp_samples[i] != 0) {
            printf("%5zu   %7d   %7d\n", i, verilog_samples[i], cpp_samples[i]);
            preview_count++;
        }
    }
}