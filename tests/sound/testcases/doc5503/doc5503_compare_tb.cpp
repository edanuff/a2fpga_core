...
int main(int argc, char** argv) {
    ...
    // Set up oscillator 0 in both implementations
    // Register addresses:
    ...
    // Set frequency (0x0120 = moderate frequency)
    write_register(doc_harness, es5503, 0x00, 0x20, &sim_time, &clk, tfp);
    write_register(doc_harness, es5503, 0x20, 0x01, &sim_time, &clk, tfp);
    
    // Set volume (0x7F = medium volume)
    write_register(doc_harness, es5503, 0x40, 0x7F, &sim_time, &clk, tfp);
    
    // Set wavetable pointer (0x00 = start at page 0)
    write_register(doc_harness, es5503, 0x80, 0x00, &sim_time, &clk, tfp);
    
    // Set control (0x10 = Free running mode, no halt, channel 1 - left)
    write_register(doc_harness, es5503, 0xA0, 0x10, &sim_time, &clk, tfp);
    ...
    for (int i = 0; i < MAX_SIM_TIME; i++) {
        ...
        if (clk) {
            ...
            if ((sim_time % SAMPLE_COLLECT_INTERVAL) == 0) {
                ...
                // Collect C++ output - we need to generate samples periodically
                if (audio_buffer_pos >= audio_buffer_size) {
                    // Process and add samples from the buffer
                    for (int j = 0; j < audio_buffer_size; j++) {
                        cpp_samples_left.push_back(audio_buffer[j * 2]);
                        cpp_samples_right.push_back(audio_buffer[j * 2 + 1]);
                    }
                    audio_buffer_pos = 0;
                }
                
                // Generate more samples for the C++ implementation
                if (audio_buffer_pos == 0) {
                    es5503->generate_audio(audio_buffer, audio_buffer_size);
                }
                
                // Use the next sample from the buffer
                audio_buffer_pos++;
    ...
    generate_wav_file(verilog_samples_left, "verilog_output_left.wav");
    generate_wav_file(verilog_samples_right, "verilog_output_right.wav");
    generate_wav_file(cpp_samples_left, "cpp_output_left.wav");
    generate_wav_file(cpp_samples_right, "cpp_output_right.wav");
