...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        // Apply gain to input
        normal_result[i] = normal_audio[i] * gain;
        
        // Clip if necessary
        if (normal_result[i] > 32767) normal_result[i] = 32767;
        else if (normal_result[i] < -32768) normal_result[i] = -32768;
    }
    ...
    write_wav_file("doc5503_normal_input_fixed.wav", normal_audio, sample_rate);
    write_wav_file("doc5503_normal_output_fixed.wav", normal_result, sample_rate);
    write_wav_file("doc5503_loud_input_fixed.wav", loud_audio, sample_rate);
    write_wav_file("doc5503_loud_output_fixed.wav", loud_result, sample_rate);
