...
| micro-SD storage (`/sdcard`), `wifi.txt` config | Gone. WiFi config → NVS (settings v4 already supports it). Disk images → LittleFS on the 8 MB flash (.dsk = 140 KB, dozens fit) uploaded via FTP/telnet, or USB-MSC from a PC later. |
