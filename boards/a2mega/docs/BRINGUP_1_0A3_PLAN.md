...
| micro-SD storage (`/sdcard`), `wifi.txt` config | Gone. WiFi config → NVS (settings v4 already supports it). Disk images → LittleFS (~1.9 MB `no_ota` partition; .dsk = 140 KB, ~a dozen fit) uploaded via FTP/telnet, or USB-MSC from a PC later. An uncompressed FPGA bitstream (~2.6 MB) does NOT fit — menu FPGA-update needs compression/streaming; JTAG from a PC is unaffected. |
| ESP32 module | Changed: 1.0a3 uses the **ESP32-S3-MINI-1-N4R2** (4 MB flash + **2 MB quad PSRAM**, same module as the a2p25), not the N8 (8 MB, no PSRAM). Less flash, but PSRAM for large buffers (disk caching, future DOC-on-ESP32). Build FQBN differs per rev — the Makefile selects it from `BOARD_REV`. |
