...
| 46 | 08-19 | B1 | f70af283 HYBRID | HEAD+net | Ugreen | captive | - | N | hub-fed | 6.5 | POWER-CYCLE fresh boot, mux IR-measured 30C | **~20s battle (Y:FF, 15+ rounds) then stable C:0177** — user-observed 'came up after about 20 seconds' | Even cool, Ugreen acquisition margin is thin — but hybrid converts thin margin into a bounded delay instead of permanent dark. Product framing: worst case = picture in ~20s. NOTE: 30C IR reading far below the 50-60C dissipation estimate — thermal model needs measured revision (emissivity caveat; measure during retrain-storm too) |
