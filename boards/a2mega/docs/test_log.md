...
| 29 | 08-18 | B1+B2 | e030f96a | HEAD+net | Ugreen hub, UtechSmart hub, USB->HDMI dongle, Anker | captive | - | N | JTAG 5V | 6.5 | multi-sink sweep | **ALL CONVERTER SINKS FAIL both boards** (Anker: B1 sometimes/flappy; Ugreen: neither; UtechSmart: neither + NO HPD; dongle: neither) | **VERDICT REVISED: design-wide marginality vs converter-class sinks, NOT a carrier-2 defect.** B1/B2 = unit variance straddling the bar on ONE hub model. Mechanism (short reflective captive channels + bridge-grade RX vs long lossy cable + monitor RX) fits ALL data. UtechSmart no-HPD = likely the known pin-assignment-D firmware gap (find_dp_mode). WS4 (Gowin supported stack) = THE road for hub viability |
