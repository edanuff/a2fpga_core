...
| 2026-09-02 | BolAAzuL USB-C->DisplayPort multi-function adapter dongle (active, us-powered) | — (no Discover Modes) | — | — | ? | ? | **UNMEASURED — PD-SILENT (2nd)**: identical signature to the Cable Matters HDMI 2.1: attach on CC1 as SRC/DFP, one `PD HARD RESET; CYCLING 5 V SOURCE`, then parked in `SOURCE-WAIT-REQUEST` with ZERO further PD events in 25 s. ⚠ LEAD ON OUR SIDE: the stack's 12-Source_Caps give-up (`releasing (non-PD sink or phantom)`) never fires and no resends are seen — its guard is `!tx_busy`, so a Source_Caps whose TXSENT/RETRYFAIL interrupt never arrives after the hard-reset/VBUS-cycle leaves `tx_busy` set and the stack goes silent. Pattern: both PD-silent devices are active adapters POWERED BY US (card = source); every negotiating device sourced VBUS to us or was a passive cable -> slow-booting partner + first unanswered Source_Caps + stuck tx_busy fits. Needs a PD event-trace build to confirm |
