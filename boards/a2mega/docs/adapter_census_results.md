...
| 2026-09-02 | BolAAzuL USB-C->DisplayPort multi-function adapter dongle (active, us-powered) | — (no Discover Modes) | — | — | ? | ? | **UNMEASURED — PD-SILENT (2nd)**: identical signature to the Cable Matters HDMI 2.1: attach on CC1 as SRC/DFP, one `PD HARD RESET; CYCLING 5 V SOURCE`, then parked in `SOURCE-WAIT-REQUEST` with ZERO further PD events in 25 s. PD EVENT TRACE (telnet `t`, fw 3118ade7+) SETTLED IT — the stuck-`tx_busy` hypothesis is REFUTED and the device is NOT silent: `TX Source_Caps -> TXOK -> RX Request -> TX Reject -> release/VBUS off -> re-attach -> Source_Caps -> partner HARD RESET -> VBUS cycle -> repeat` (~1 s loop; the salvaged first trace showed only the RETRY_FAIL storm of its post-reset boot window). Root cause is OUR RDO acceptance rule: `usb_pd_fixed_rdo_is_acceptable()` also required `maximum_ma <= 1000`, rejecting every sink that sets Capability Mismatch (spec: operating current stays within the PDO, MAX carries the sink's wish). Fix in fw (41b008d8): enforce operating current only; RDO fields traced. SECOND PERSONA seen when the card BOOTS bus-powered by the adapter (BolAAzuL sources VBUS; card = SNK/UFP): contract completes in 200 ms (Source_Caps/Request/Accept/PS_RDY) but our DR_Swap goes UNANSWERED -> we stay UFP -> no DP discovery. Bench-only (in a slot the card is source). Fix verification needs the source persona: slot power on, hot-attach |
...
Result (n=8 devices): the only 4-lane sink at both gates is a REAL DP MONITOR over a direct USB-C->DP cable (PASS). Every hub-class adapter is a 2-lane HBR2 converter behind a 4-lane PD advertisement (720p ceiling for RBR-only). Three devices are unmeasured: two AUX-offset-blind on 1.0a3 (generic 4K dongle, Fangor USB-C monitor) and two PD-silent (Cable Matters HDMI 2.1, USB-C->DP adapter #2) — both us-powered active adapters; a stuck-`tx_busy` park in our stack is the leading hypothesis. Readability is per-sink: the direct DP monitor and all hubs read; the Fangor and the dongle do not. Side finding with product impact: our PD stack rejected pin assignment E, so direct USB-C->DP cables never worked on a2mega until fw 79538724.
