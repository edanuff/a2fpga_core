...
Result (n=6 devices): the only 4-lane sink at both gates is a REAL DP MONITOR over a direct USB-C->DP cable (PASS). Every hub-class adapter is a 2-lane HBR2 converter behind a 4-lane PD advertisement (720p ceiling for RBR-only). Two devices are unmeasured for unrelated reasons (offset-blind dongle; PD-silent Cable Matters). Side finding with product impact: our PD stack rejected pin assignment E, so direct USB-C->DP cables never worked on a2mega until fw 79538724.
