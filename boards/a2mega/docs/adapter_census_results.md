...
| 2026-09-02 | Fangor USB-C portable monitor (USB-C input; monitor sources VBUS -> card SNK; first CC2-orientation device) | 00000405 | C / "4-LANE OK" | 0000 (never read) | ? | ? | **UNMEASURED — AUX offset-blind**: PD `DP-ACTIVE`, `MUX=1 HPD=1`, mux regs consistent for CC2 (`0x0A=1E`, `0x13=A0`), FPGA sees HPD, but ZERO AUX reply edges (`E:00`) in both crossbar states (`f`) and after retrain. Same class as the generic 4K dongle; 1.0a4 receiver bias. (One `DP ALT MODE RESPONSE TIMEOUT` during PD, recovered.) |
