...
Result (n=8 devices): TWO 4-lane sinks at both gates — the Sceptre DP monitor over a direct USB-C->DP cable and the BolAAzuL USB-C->DP dongle (both PASS, both needed PD-stack fixes to negotiate at all). Every hub-class adapter is a 2-lane HBR2 converter behind a 4-lane PD advertisement (720p ceiling for RBR-only). Three devices are unmeasured: two AUX-offset-blind on 1.0a3 (generic 4K dongle, Fangor USB-C monitor) and one still PD-blocked (Cable Matters HDMI 2.1 — same Request->Reject->HardReset loop predicted; re-test pending on fw be2ab629). Readability is per-sink: the direct DP monitor and all hubs read; the Fangor and the dongle do not. Side finding with product impact: our PD stack rejected pin assignment E, so direct USB-C->DP cables never worked on a2mega until fw 79538724.
