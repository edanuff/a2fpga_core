...
| 0.3 | Answer open hardware questions (§7): /VP direction, Bank 5 VCCIO, E/MLB connectivity, BE behavior during IIgs DMA | Updated twgs docs; go/no-go for Phase 1 power-on |
...
| 5.2 | DMA: on `GS_BE` deassert (and/or slot `a2_dma_n`), release bus within spec, pause write-FIFO drain, snoop address/data during DMA to invalidate shadow pages touched (selective invalidation per TWGS §8); conservative fallback: invalidate all shadowed RAM on DMA burst |
...
1. **/VP direction (potentially needs a board rev).** The schematic routes `GS_VP`
   *into* the FPGA through U8's input-only group — but VPB is a **CPU output**; the
   IIgs FPI consumes it for interrupt vector handling. As drawn, nothing drives /VP
   toward the motherboard (it will float/pull-up). Verify against the IIgs schematic
   whether the FPI actually requires /VP from the CPU socket and whether interrupts
   misbehave without it. Mitigations: board rev swapping /VP to a driven pin, or
   piggyback on an unused driver.
2. **Bank 5 VCCIO conflict.** GS control pins (PH2, IRQ, ABORT, RDY, VP, and all
...
3. **E / MLB / VDA / VPA at the socket.** VDA/VPA/MX appear no-connect on J8. The
   65816's E output *is* used by the IIgs (M-state/E-state handling). Confirm
   whether J8 pin "E" is wired to the FPGA or the cable, and whether the FPGA must
   drive it (the soft core tracks E internally; we need a path to present it).
4. **BE behavior during IIgs DMA.** We assume the motherboard deasserts BE at the
   CPU socket during slot DMA. Verify on real hardware (Phase 1 monitor) — if BE is
   strapped high on the IIgs, DMA detection must come from the slot-side `a2_dma_n`
   daisy chain instead, with conservative timing.
5. **Bus parking while accelerated.** Unverified assumption that the FPI tolerates
...
6. **CDC latency vs FPI setup expectations** (§ Phase 2.2). If 54 MHz sampling is
...
7. **Core accuracy edge cases:** decimal mode, emulation-mode quirks, open-bus
   reads, interrupt timing. Mitigated by the Phase 2.5 opcode/trace test suites.
8. **Licensing** of the imported core (§3.2 item 6).
9. **Dual-role interactions:** the same FPGA is a slot peripheral (drives `a2_d`,
