...
module tb_afe_adjust_closedloop_perlane;  // ADAPTED copy of sim/tb_afe_adjust_closedloop.v (HEAD) for the per-lane ports
    ...
    initial begin
        ...
        if (train_set_byte !== 16'h0606) begin
            errors = errors + 1;
            $display("FAIL: post-training train_set_byte %04x want 0606 (idle)",
                     train_set_byte);
        end
