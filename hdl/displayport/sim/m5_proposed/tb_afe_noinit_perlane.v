...
`timescale 1ns/1ps
module tb_afe_noinit_perlane;  // ADAPTED copy of sim/tb_afe_noinit.v (HEAD) for the per-lane ports (request replicated to both lanes, lane0 byte checked)
    ...
    wire [15:0] byte16; wire [7:0] byte_o = byte16[7:0]; wire busy; wire [5:0] dbg;
    ...
    afe_adjust_seq #(.ENABLE_AFE_ADJUST(1), .NUM_LANES(2),
        ...
        .train_set_byte(byte16), .afe_busy(busy), .dbg_afe(dbg), .dbg_afe1(),
