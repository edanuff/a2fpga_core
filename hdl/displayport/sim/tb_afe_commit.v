...
module tb_afe_commit;
    ...
    initial begin
        ...
        #500;
        if (!busy || byte_o !== 8'h06) begin errors = errors + 1; $display("FAIL A1: mid-INIT busy=%0d byte=%02x (want 1/0x06)", busy, byte_o); end
        else $display("  ok A1: INIT in flight -> busy=1, declaration 0x06");
        #(20_000);
        if (busy || wr_cnt != 8 || byte_o !== 8'h06 || !dbg[4]) begin errors = errors + 1; $display("FAIL A1b: after INIT busy=%0d wr=%0d byte=%02x known=%0d", busy, wr_cnt, byte_o, dbg[4]); end
        ...
        if (!busy || byte_o !== 8'h06) begin errors = errors + 1; $display("FAIL A2: in-flight declaration changed early (busy=%0d byte=%02x)", busy, byte_o); end
        else $display("  ok A2: VS1 in flight -> still declares 0x06, busy=1");
        #(20_000);
        if (busy || byte_o !== 8'h01 || wr_cnt != 16) begin errors = errors + 1; $display("FAIL A2b: after VS1 byte=%02x wr=%0d", byte_o, wr_cnt); end
        ...
        if (busy || byte_o !== 8'h01 || !dbg[5]) begin errors = errors + 1; $display("FAIL A3: failed seq: busy=%0d byte=%02x err=%0d (want 0/0x01/1)", busy, byte_o, dbg[5]); end
        ...
        if (wr_cnt != 16 || byte_o !== 8'h01) begin errors = errors + 1; $display("FAIL B1: retention: wr=%0d byte=%02x", wr_cnt, byte_o); end
        ...
        if (byte_o !== 8'h06) begin errors = errors + 1; $display("FAIL B2: after reinit byte=%02x (want legacy 0x06)", byte_o); end
        #(20_000);
        if (wr_cnt != 24 || wr_data[16] !== {16'd0, 4'd13, 12'd0} || byte_o !== 8'h06) begin errors = errors + 1; $display("FAIL B2b: re-baseline: wr=%0d data16=%08x byte=%02x", wr_cnt, wr_data[16], byte_o); end
