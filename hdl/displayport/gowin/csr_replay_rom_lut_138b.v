// Per-die CSR replay ROM, GW5AST-138B. Same module name as the 60B
...
// transceiver bank (right in the congested GTR12 corner — the cm_life
// upar_arbiter family's neighborhood) and needed cm_life multicycle
// crutches on the replay_idx->drp_addr cone. Now a sync-read block
// ROM: contents from csr_replay_rom_138b.hex (generated alongside the
// .svh by boards/a2mega/tools/gen_csr_replay.py — always regenerate
// BOTH on IP regen), length still from the .svh localparam. The +1
// read latency is absorbed by the replay FSM's inter-op gap:
...
module csr_replay_rom_lut(
    ...
    `include "csr_replay_rom_138b.svh"

    (* syn_romstyle = "block_rom" *) reg [55:0] rom_r [0:511];
    initial $readmemh("csr_replay_rom_138b.hex", rom_r, 0);
